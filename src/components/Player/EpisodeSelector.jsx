...
export default function EpisodeSelector({ seriesId, numSeasons, onPlay }) {
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {Array.from({ length: numSeasons }, (_, i) => i + 1).map(s => (
          <button
            ...
            style={{
              background: s === season ? 'var(--accent)' : 'var(--bg-surface)',
              color: s === season ? '#fff' : 'var(--text-secondary)',
              border: `1px solid ${s === season ? 'var(--accent)' : 'var(--border)'}`,
              boxShadow: s === season ? '0 0 16px var(--accent-glow)' : 'none',
            }}
          >
            Season {s}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-36 rounded-xl shimmer" style={{ border: '1px solid var(--border)' }} />
          ))
        ) : (
          episodes.map(ep => (
            <motion.div
